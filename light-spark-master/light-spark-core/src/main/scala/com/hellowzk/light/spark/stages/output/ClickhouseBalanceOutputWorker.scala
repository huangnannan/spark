...
import com.hellowzk.light.spark.beans.output.ClickhouseBalanceOutputConfig
 ...
 *
 **/
class ClickhouseBalanceOutputWorker extends BaseWorker {
  override def process(config: BaseConfig)(implicit ss: SparkSession): Unit = {
    val item = config.asInstanceOf[ClickhouseBalanceOutputConfig]
    // 执行前置sql逻辑，一般为删除操作
    ...
    item.tables.foreach { case (src, dist) =>
      logger.info(s"Clickhouse BalancedClickhouseDataSource output, start save '$src' to '$dist'...")
      val t1 = System.currentTimeMillis()
      val dfWriter = ss.table(src).write.mode(item.mode).format(classOf[ClickHouseBalancedRelationProvider].getName)
      filterd.put("dbtable", dist)
      dfWriter.options(filterd).save()
      logger.info(s"Clickhouse BalancedClickhouseDataSource output, save '$src' to '$dist' success cost ${System.currentTimeMillis() - t1} ms.")
    }
  }
   ...
   * @param item JDBC 配置
   */
  def executePreSQL(item: ClickhouseBalanceOutputConfig): Unit = {
    logger.info(s"Clickhouse Cluster connect url: ${item.url}")
