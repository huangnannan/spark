...
import org.apache.spark.sql.types.{ArrayType, BinaryType, BooleanType, ByteType, DataType, DateType, Decimal, DecimalType, DoubleType, FloatType, HIVE_TYPE_STRING, IntegerType, LongType, Metadata, MetadataBuilder, ShortType, StringType, StructField, StructType, TimestampType}
 ...
 * 作者： zhaokui
 * <p>
 * 通过 BalancedClickhouseDataSource 创建 clickhouse 集群的 conn
 **/
object ClickHouseUtils extends Logging {
  ...
  def createConnectionFactory(options: JDBCOptions): () => Connection = {
    () => {
      val clickhouseProperties = new ClickHouseProperties(options.asConnectionProperties)
      val balanced = new BalancedClickhouseDataSource(options.url, clickhouseProperties)
      balanced.actualize()
      val conn: Connection = balanced.getConnection()
      conn
    }
  }
  ...
  def tableExists(conn: Connection, options: JDBCOptions): Boolean = {
    ...
    Try {
      val sql = dialect.getTableExistsQuery(options.table)
